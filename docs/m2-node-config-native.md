...
Review the stopped-state checkpoint before restarting. Recreate Seller, Buyer and Carrier **one at a time** with separate `docker compose ... up -d --no-deps --force-recreate peer0-<org>` native commands, retaining each named ledger and taking separate mode-0600 logs. A plain `start` could reuse an old bind source. Require a new running ID, old ledger name/creation, own read-only MSP/TLS/config/deny-builder mounts, exact networks, no socket/ports, and startup log lines deploying all three in-process system chaincodes without `cscc.syscc` package lookup. Recheck own-DNS 7051 TLS, own-client `/healthz` HTTP 200/OK and no-client TLS rejection. The Seller-only native prejoin `peer channel list` may then prove CSCC readiness before Buyer/Carrier restart; its empty result is not channel readiness. Run full T-NET-02 against **current** containers after all three recreate. #18 owns individual Peer joins and post-join checks. Preserve the original proposal status-500 log and do not claim a committed txId or Peer height from that failed proposal.

## Seller post-join delivery mTLS repair

Seller has joined `supplychannel` locally, but its Orderer delivery TLS handshake failed because `peer.tls.clientAuthRequired:false` omitted the Peer TLS client certificate. The separate [Peer delivery mTLS repair runbook](m2-peer-client-mtls.md) pins the new 7051 trust roots, one-shot CLI client pair, exact staged-config delta and preservation gates for its joined ledger. Buyer/Carrier joins remain on hold until the repair has passed independent live review. A local Seller height-1 query is not evidence of ongoing block delivery.
...
3. After #18 joined all three Orderers individually, the three CouchDBs and Peers started with retained named volumes; their historical failures and repairs are recorded above. Seller alone has joined its local `supplychannel` ledger; its post-join delivery mTLS repair is documented separately. Buyer/Carrier Peer joins and network-level readiness remain #18 work. Do not call a process that merely listens on 7051 ready for `supplychannel` or `supplycc`.
...
Record T-NET-02 as **NOT RUN** until these live checks succeed against the current containers. The three-node admin mTLS matrix and full §20.1 `T-NET-04` are **PASS** in [#18 evidence](../evidence/m2/issue-18.md); Seller's local join/height 1 is also recorded there, followed by delivery TLS **FAIL**. Buyer/Carrier joins, current channel configuration, three-organization equal-height block-hash comparison and chaincode/transaction checks remain **NOT RUN** until their owners execute them. The canonical `genesisHash` field stays NOT RUN until NET-08 records the real block-0 header identity; the matching fetched block files' SHA-256 is an artifact checksum. No txId or validation code is invented for offline preflight.
