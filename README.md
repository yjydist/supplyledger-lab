...
make test-m2-nodes      # exercise node preflight using temporary private files after M1
...
The [M2 topology guide](docs/m2-compose-topology.md) explains the base Orderer/Peer/CouchDB overlay and its [static evidence](evidence/m2/issue-15.md). `make network-config` parses the merged Compose model and checks the pinned images, network membership, mounts and separate state volumes without starting a node. The [native channel block runbook](docs/m2-channel-native.md) records the first `configtxgen` and decode steps; `make verify-m2-initial-block` audits that ignored block against the existing M1 public MSPs and Orderer TLS certificates. [#18's three-Peer checkpoint](evidence/m2/issue-18.md#three-peer-same-height-block-0-checkpoint-net-06) records local block-0 fetches and equal-height canonical header hashes; the Peer-fetched block files differ from the original by one metadata byte. The runbook's NET-08 section gives separate Orderer-admin and Peer-local current-CONFIG fetch commands; [#17's live checkpoint](evidence/m2/issue-17.md#six-node-live-current-config-read-only-checkpoint) records six native fetches and pinned decodes with an approved block-0 CONFIG comparison. `make test-m2-current-config` runs synthetic decoded-structure controls; `make verify-m2-current-config` compares six decoded JSON inputs with that approved decoded baseline. The verifier does not independently check raw block data hashes or raw-block-to-JSON provenance; the live NET-08 / `T-NET-03` configuration PASS rests on the retained native raw fetches, pinned decodes, source/output hashes and independent review. Full live MSP enforcement, post-genesis block delivery and Discovery remain `NOT RUN`. The [node runbook](docs/m2-node-config-native.md) prepares ignored node configs and per-organization CouchDB credentials; `make test-m2-nodes` checks this preflight in temporary directories. The three CCaaS declarations and self-built image belong to M3 [#22](https://github.com/yjydist/supplyledger-lab/issues/22); live Docker inspection and full `T-NET-02` belong to M2 [#16](https://github.com/yjydist/supplyledger-lab/issues/16). `make network-up` and `make channel-create` (M2), `make chaincode-deploy` (M3), and later stage targets still exit `NOT RUN` until their first native steps are recorded.
