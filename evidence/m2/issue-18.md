...
Issue [#18](https://github.com/yjydist/supplyledger-lab/issues/18) remains **OPEN**. The first checkpoint records the three Orderer admin mTLS matrices and the first Orderer0 join failure. After the independently reviewed #16 request-size repair, all three Orderers joined the same retained block individually and became active consenters. **`T-NET-04` PASS** at its specified Orderer join/admin mTLS scope. Seller, Buyer and Carrier later joined their local channel ledgers individually at height 1; a subsequent 33-second, three-Peer read-only checkpoint found the same canonical block-0 header hash at the same local height. Seller's original delivery connection failed at TLS client authentication; #16 subsequently repaired Peer client mTLS, while receipt of a new block remains unverified. Anchors, cross-organization discovery and current live configuration remain **NOT RUN**.

- Tests: `M2-I18-ADMIN-01` (live three-node admin mTLS matrix) **PASS**; `M2-I18-NET05-01` (Orderer0 first native join) **FAIL** at admin request parsing; `M2-I18-NET05-02` (Orderer0 retry and status) **PASS**; `M2-I18-NET05-03` (Orderer1/2 individual joins and block-0 agreement) **PASS**; `M2-I18-NET06-02` (Seller local Peer join) **PASS**; `M2-I18-NET06-03` (Seller's original live block delivery) **FAIL** at TLS client authentication; `M2-I18-NET06-04` (Buyer local Peer join) **PASS**; `M2-I18-NET06-05` (Carrier local Peer join) **PASS**; `M2-I18-NET06-06` (three-Peer same-height block-0 check) **PASS** at height 1. `T-NET-04` **PASS** for the exact §20.1 Orderer/admin scope; native local Peer joins and the limited block-0 comparison are complete, while new-block delivery and NET-07/08 remain pending. See `SPEC.md` §§5.4, 19.1, 20.1 and the dedicated-client requirement transferred from #12.
...
Carrier's local height-1 `currentBlockHash` matches the prior Seller and Buyer observations and #17's offline canonical block-0 header hash (`5c68bf099e95b934b6034a4d7aa8c52442751c30bc8b56fcbbde9b0138e38ba7`). At this Carrier-only snapshot these were individual observations, before the later three-Peer checkpoint below. No post-genesis ordered block, delivery receipt, committed transaction ID or validation code is inferred from these CSCC calls. Anchor/discovery checks, current-config NET-08 and full `T-NET-03` were **NOT RUN**; #18 remains OPEN. The prior Seller delivery TLS **FAIL** remains in its historical section, while #16's later repair has not yet proven receipt of a new ordered block.

## Three-Peer same-height block-0 checkpoint (NET-06)

- Test `M2-I18-NET06-06`; `SPEC.md` §§5.4 NET-06, 19.1 and M2 exit. Execution source: clean local/remote main `7b5dfac86dcf36aa5b21faaa4d6391365861c0c2`; `versions.lock.yaml` SHA-256 `422fba14294aaf9f0c40862fcd112ed3d2ed664cd5294c7bfe0aa14477fa234b`. Compose tier: `bootstrap.yaml` + `ca.yaml` + `network.yaml`, project `supplyledger`, `bootstrap` and `ca` profiles. All three prior native joins and their separate evidence had been independently reviewed before this read-only checkpoint. No configuration transaction or network restart occurred.
- The original ignored #17 block remained mode `0600`, 27,946 bytes, with **file** SHA-256 `b0a5ec0894d45ca7b6577b8f576d176347ad90cb4f80ac18de2dea3cc5ebd08a`. Its #17 canonical block-header digest was `5c68bf099e95b934b6034a4d7aa8c52442751c30bc8b56fcbbde9b0138e38ba7`. These are different hash definitions. Each native tool container used only its organization's network, rendered `core.yaml`, admin ECert MSP, own `peer0-tls` MSP and TLS root, exact `peer0.<org>.supply.test:7051`, `CORE_PEER_TLS_CLIENTAUTHREQUIRED=true`, nonroot/read-only filesystem, and no Docker socket, other-organization private key or TLS hostname override.
- Six separate native `peer` calls were recorded with exact mode-`0600` `.command.txt`, `.log`, `.exit`, `.started.utc` and `.ended.utc` under ignored mode-`0700` `.runtime/m2-issue18-equalheight/`. The 42 retained files in that directory, including fetched blocks, decoded JSON, audit and structured inspect output, were all mode `0600`. The three `getinfo` calls finished within UTC `2026-09-25T17:47:35Z–17:48:08Z`. Each returned exit **0**, local `height=1` and `currentBlockHash=XGi/CZ6VuTS2A0pNeqjFJEJ1HDC8i1b8u96bATjji6c=`. This establishes the same canonical block-header hash at the same local height in that 33-second observation window; it does not imply all ledgers will always stay synchronized.

The exact Seller `getinfo` command form was repeated **individually** for Buyer and Carrier, replacing each occurrence of `seller` and `SellerMSP` with that organization's own values. The adjacent raw `.command.txt` records preserve each literal invocation:
...
docker run --rm --pull=never --platform linux/arm64 --network supply-seller --read-only --tmpfs /tmp --user "$(id -u):$(id -g)" \
  ...
  -e CORE_PEER_TLS_ROOTCERT_FILE=/run/supply/peer-tls-root.pem \
  --entrypoint peer supply-tools:m0-fabric3.1.5-ca1.5.22 channel getinfo -c supplychannel \
  > .runtime/m2-issue18-equalheight/seller-equal-height-getinfo.log 2>&1
```

Each subsequent `peer channel fetch 0` omitted `-o` and any Orderer address environment, so pinned Fabric v3.1.5 [`fetch.go`](https://github.com/hyperledger/fabric/blob/v3.1.5/internal/peer/channel/fetch.go#L47-L58) selected the calling organization's **Peer Deliver** service. The Seller command below was again repeated as separate Buyer/Carrier native calls with only same-organization names/paths changed. `umask 077` made the three fetched block files mode `0600`:
...
docker run --rm --pull=never --platform linux/arm64 --network supply-seller --read-only --tmpfs /tmp --user "$(id -u):$(id -g)" \
  ...
  --mount type=bind,src="$PWD/.runtime/identities/seller/seller-peer0-tls/msp",dst=/run/supply/tls,readonly \
  --mount type=bind,src="$PWD/.runtime/trust/seller-tls-ca.pem",dst=/run/supply/peer-tls-root.pem,readonly \
  --mount type=bind,src="$PWD/.runtime/m2-issue18-equalheight",dst=/run/evidence \
  -e FABRIC_CFG_PATH=/etc/hyperledger/fabric \
  -e CORE_PEER_LOCALMSPID=SellerMSP -e CORE_PEER_MSPCONFIGPATH=/run/supply/msp \
  -e CORE_PEER_ADDRESS=peer0.seller.supply.test:7051 -e CORE_PEER_TLS_ENABLED=true \
  -e CORE_PEER_TLS_CLIENTAUTHREQUIRED=true \
  -e CORE_PEER_TLS_ROOTCERT_FILE=/run/supply/peer-tls-root.pem \
  --entrypoint sh supply-tools:m0-fabric3.1.5-ca1.5.22 -ceu \
    'umask 077; exec peer channel fetch 0 /run/evidence/seller-peer-block0.block -c supplychannel' \
  > .runtime/m2-issue18-equalheight/seller-peer-fetch-block0.log 2>&1
```

| Organization | `getinfo` UTC start; raw log SHA-256 | Peer fetch-0 UTC start; raw log SHA-256 | Native results |
| --- | --- | --- | --- |
| Seller | `17:47:35Z`; `b7a74bb291659aec78318cf9e1e9763eed1495ecbf1a20f1b32f30ca4741f117` | `17:48:32Z`; `567f5db53a27b2ce57951d55f8cbbb15aeffb280c0bb493378047af857d7783b` | both exit 0; height 1/hash above; `Received block: 0` |
| Buyer | `17:47:52Z`; `f1f46fa6b7266114c90e3fed2b749504ad76ce7bd160184cdf88f7f0a50b2572` | `17:48:54Z`; `9823ce6e843a68796f1a807355429a194d811556dff45b73f793f566214102ec` | both exit 0; height 1/hash above; `Received block: 0` |
| Carrier | `17:48:08Z`; `ed17e8879ae8d742a546c32c2da8b9e6138bd291ee118c83793540b3dc9a59e1` | `17:49:14Z`; `bc0ff7de0685d8fc81e2dd850661dd7ff96ac11477f9f23782761b7d6278f439` | both exit 0; height 1/hash above; `Received block: 0` |

The three Peer-fetched `.block` files were byte-identical to **one another**: 27,947 bytes and file SHA-256 `042d7e1c2cf18b83bc792575414ed7ff2b811300c718a9ac00c3320919da7137`. Their file bytes differ from the original #17 block (27,946 bytes, `b0a5…d08a`). Pinned `configtxlator proto_decode --type common.Block` of the Seller-fetched copy, followed by a read-only JSON comparison, found **equal block header and data** with the original; only metadata slot 2 changed from empty to one byte. The [`protoutil.BlockHeaderHash`](https://github.com/hyperledger/fabric/blob/v3.1.5/protoutil/blockutils.go#L39-L62) ASN.1 DER calculation from that fetched header gave the same canonical block-0 digest `5c68bf…e38ba7`, exactly matching all three native `getinfo` values. Raw decoded JSON and the `block0-header-audit.log` (SHA-256 `5ef42bd0c623cb7f9f396b02829a69482089644a8c0fc6ab728ccb55e0e5495e`) remain ignored; no full MSP/certificate JSON is committed. A structured Docker inspect after these calls found the same three Peer container IDs and original named ledger volumes as the read-only pre-check; the after snapshot is retained under the ignored directory.

**PASS** for `M2-I18-NET06-06`: three distinct founding Peers exposed the same block-0 header hash at local height 1 in this observation window. **NOT RUN** for a newly ordered block reaching all three Peers, anchor/externalEndpoint/gossip discovery NET-07, current live config NET-08 and full `T-NET-03`; no new committed `txId` or validation code was produced by these read-only calls. #18 remains OPEN pending its remaining M2 checks.
